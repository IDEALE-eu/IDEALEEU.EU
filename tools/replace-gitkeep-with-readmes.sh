...
ROOT="${1:-.}"
FORCE="${FORCE:-0}"          # set FORCE=1 to overwrite existing README.md
DRY="${DRY:-0}"              # set DRY=1 to preview without writing
YEAR="$(date +%Y)"
...
mapfile -t FILES < <(find "$ROOT" -type f -name .gitkeep | sort)
...
hr
echo "Done. Updated: $CREATED | Skipped: $SKIPPED"
